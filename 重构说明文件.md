...
- `Controllers/EquipmentController.cs`
  - `CreateEquipmentStateContext()` - 新增
  - `GetAvailableActions()` - 重构
  - `OperateEquipment()` - 重构
  - `CreateOrder()` - 重构
  - `ConfirmRepair()` - 重构

**注**: 其他Controller (StoreController, VenueEventController等) 可以参考EquipmentController的重构方式进行类似重构。
...
1. **ParkingController**: 车位状态管理可以完全使用ParkingSpace状态模式
2. **StoreController**: 店铺状态变更申请和审批流程
3. **VenueEventController**: 活动状态管理的完整重构
4. **CashFlowController**: 账单支付状态管理
...
2. **重构了4个核心方法**: EquipmentController中的关键方法
3. **消除了大量条件判断**: 替换为优雅的状态模式
4. **提高了代码质量**: 符合SOLID原则,易于维护和扩展
